define(["postmonger"], function (Postmonger) {
  ...
  function initialize(data) {
    ...
    // If there is no offerPreference selected, disable the next button
    if (!offerPreference) {
      showStep(null, 1);
      connection.trigger("updateButton", { button: "next", enabled: false });
      // If there is a offerPreference, skip to the summary step
    } else {
      $("#offerPreference")
  ...
  function save() {
    var name = $("#offerPreference").find("option:selected").html();
    var value = getOfferPreference();
  ...
  function getOfferPreference() {
    return $("#offerPreference").find("option:selected").attr("value");
  }
});